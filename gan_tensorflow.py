...
def show_result(num_epoch, show=False, save=False, path='result.png'):
    z_ = np.random.normal(0, 1, (25, 100))  # input of generator
    ...
    for i in range(5):
        for j in range(5):
            axes[i, j].cla()
            axes[i, j].imshow(np.reshape(test_images[iterator], (28, 28)), cmap='gray')
            iterator += 1

    label = 'Epoch %s' % num_epoch
...
if not os.path.isdir('MNIST_GAN_results/results2'):
    os.mkdir('MNIST_GAN_results/results2')
train_hist = {}
train_hist['D_losses'] = []
train_hist['G_losses'] = []
...
for epoch in range(train_epoch):
    ...
    p = 'MNIST_GAN_results/results2/MNIST_GAN_' + str(epoch + 1) + '.png'
